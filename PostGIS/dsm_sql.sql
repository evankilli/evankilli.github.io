...
-- so i guess this one is just leaving the geometries intact, the others are getting rid of it
...
FROM flood;
--how many geometry parts do each of these features contain? anywhere from around 300 to 600
--and are their geometries valid? If not, many GIS algorithms will not work! it looks like the geometries are invalid
...
/* SELECT *, st_area(geom)/1000000 as area_km2
FROM flood */
...
-- this would be finding out the gender ratio of the population, and its 1 because a) the populations of each gender are roughly equal and b) there is likely some sort of rounding/sigfigs
...
-- confused by why it's only showing up as 1.... instead of showing an actual number with multiple decimals
...
SELECT reg_name, id, count(total_both) as cntwards, sum(total_both) as totalpop, sum(total_male) as male, sum(total_fema) as female, sum(total_fema)::real / sum(total_male) as genratio
FROM census
GROUP BY reg_name, id;
...
-- reg_id doesn't exist --> change to id which does actuall exist
...
--unsure exactly what this is doing? ok now it makes sense, it's checking how many times each ward appears, e.g. how many Main Streets are there in the US? Main St Manchester vs Main St Midd
...
-- duplicates definitely show up, but much less than ward codes. assuming this is like having the 8th congressional district in different states, but Florida's 8th is diff from California's 8th
...
SELECT dis_name, count(ward_name) as cntward
from census
where reg_name = 'Dar-es-salaam'
group by dist_name
order by cntward desc
-- there are three districts; Kinondoni has 34, Temeke 30, and Ilala 26
...
SELECT addgeometrycolumn('evan','ward_census','utmgeom',32737,'MULTIPOLYGON',2);
...
DROP COLUMN geom;
/* um, going to try and view the tables, I'm now getting a "database error", that supposed to happen? just needed to refresh!


/* try adding a column of type REAL to ward_census and calculating the area in square kilometers! */
select ward_name, st_area(utmgeom)/1000000 as ward_areakm2
from ward_census
-- check to make sure it works!

alter table ward_census
add column ward_areakm2 real
-- add a column

update ward_census set ward_areakm2 = st_area(utmgeom)/1000000
-- populate column w data
...
create table ward_flood
as SELECT ward_census.*, st_multi(st_intersection(ward_census.utmgeom, flooddissolve.geom))::geometry(multipolygon,32737) as geom
FROM ward_census INNER JOIN flooddissolve
ON st_intersects(ward_census.utmgeom, flooddissolve.geom);
-- basically just nesting the operation from the prior step within a "create table" function
...
select ward_name, st_area(utmgeom) as flooded_aream2
from ward_flood
-- check to make sure it works! it did!

alter table ward_flood
add column flooded_aream2 real
-- add columns
-- fuck, needed to add type!!!!

update ward_flood set flooded_aream2 = st_area(utmgeom)
-- populate column w data


/* can you join that area back to the original wards ? */
select ward_flood.*, flooded_aream2 as flooded_aream2
from ward_flood left join wards
on ward_flood.ward_name = wards.ward_name
-- check if it joins

ALTER TABLE ward_census
ADD COLUMN flooded_aream2 real
add column flooded_areakm2 real
-- add column to original ward table

UPDATE ward_census
SET flooded_aream2 = ward_flood.flooded_aream2,flooded_areakm2 = ward_flood.flooded_aream2/1000000
FROM ward_flood
WHERE ward_census.ward_name = ward_flood.ward_name
-- join in data from ward_flood to new column
-- got rid of second "set" line


/* can you calculate the percentage of area flooded and the population density now ? */
select *, flooded_areakm2/ward_areakm2 as percent_flood
from ward_census
-- check it works

alter table ward_census
add column percent_flood real
-- add column to ward census table

update ward_census
set percent_flood = flooded_areakm2/ward_areakm2
-- add data to columns

update ward_census
set percent_flood = 0
where flooded_aream2 is null
-- set null areas as 0
...
update ward_census
set pop_density = totalpop/ward_areakm2
-- data babeyyyyy
